%% STEP106_permutation_CHANNEL_ROI_FisherScore_bestModels_noLeakage.m
...
Models = table( ...
    ["channel"; "roi"; "channel"], ...
    ["binary"; "binary"; "threeclass"], ...
    ["orientation_vs_conjunction"; "color_vs_orientation"; "color_vs_orientation_vs_conjunction"], ...
    [2; 3; 3], ...
    ["retr"; "maint"; "maint"], ...
    ["lda"; "randomforest"; "randomforest"], ...
    [4; 19; 1], ...
    'VariableNames', {'UnitLevel','TaskType','Contrast','Run','Phase','Classifier','K'});

cfg.outDir = fullfile(cfg.ROOT,'_wm_STEP106_permutation_CHANNEL_ROI_FisherScore_bestModels_noLeakage');

if ~exist(cfg.outDir,'dir'), mkdir(cfg.outDir); end
diary(fullfile(cfg.outDir,'STEP106_log.txt'));

fprintf('\n=== STEP106 permutation Channel/ROI Fisher Score best models ===\n');
fprintf('Started: %s\n', datestr(now));
fprintf('nPerm = %d\n', cfg.nPerm);
disp(Models);
...
fprintf('Dataset rows=%d | candidate numeric features=%d | subjects=%d\n', ...
...
writetable(GlobalFoldTable, fullfile(cfg.outDir,'STEP106_internal_global_subject_LOO_folds.csv'));
...
fprintf('Detected channel units=%d | ROI units=%d\n',height(ChanUnits),height(ROIUnits));

if isempty(ChanUnits)
    error('No channel-level features detected. Check feature names.');
end
...
for mi = 1:height(Models)

    model = Models(mi,:);

    fprintf('\n\n============================================================\n');
    fprintf('Model %d/%d | %s | %s | %s | run=%d | phase=%s | clf=%s | K=%d\n', ...
        mi,height(Models),model.UnitLevel,model.TaskType,model.Contrast,model.Run,model.Phase,model.Classifier,model.K);
    fprintf('============================================================\n');

    try
        if string(model.UnitLevel) == "channel"
            UnitsThis = ChanUnits;
        elseif string(model.UnitLevel) == "roi"
            UnitsThis = ROIUnits;
        else
            error('Unknown UnitLevel: %s', string(model.UnitLevel));
        end

        cfgTask = cfg;
        cfgTask.taskType = char(string(model.TaskType));
        cfgTask.contrastName = char(string(model.Contrast));
        cfgTask.run = model.Run;
        cfgTask.phase = char(string(model.Phase));

        [idxTrial,yTrial,subjTrial] = get_task_trials(subjAll,condAll,runAll,phaseAll,cfgTask);

        if sum(idxTrial) < 20
            warning('Too few trials. Skipping model.');
            allStatus{end+1,1} = make_status(model,"too_few_trials"); %#ok<AGROW>
            continue;
        end
        ...
        if isempty(Folds)
            warning('No valid folds. Skipping.');
            allStatus{end+1,1} = make_status(model,"no_valid_folds"); %#ok<AGROW>
            continue;
        end

        %% Observed
        [ObsR,ObsP,ObsS] = run_model_once(Xtask,yTask,subjTask,Folds,UnitsThis,model,cfg);

        ObsR.ModelIndex = repmat(mi,height(ObsR),1);
        ObsR = movevars(ObsR,'ModelIndex','Before',1);

        allObserved{end+1,1} = ObsR; %#ok<AGROW>

        writetable(ObsP, fullfile(cfg.outDir,sprintf('STEP106_model%d_observed_predictions.csv',mi)));
        writetable(ObsS, fullfile(cfg.outDir,sprintf('STEP106_model%d_observed_selected_features.csv',mi)));

        fprintf('\nObserved:\n');
        disp(ObsR);

        %% Permutations
        permRows = cell(cfg.nPerm,1);

        for pi = 1:cfg.nPerm

            rng(cfg.rngSeed + 100000*mi + pi);

            yPerm = permute_labels_within_subject(yTask,subjTask,string(model.TaskType));

            [Pr,~,~] = run_model_once(Xtask,yPerm,subjTask,Folds,UnitsThis,model,cfg);

            permRows{pi} = table(mi,pi,Pr.Accuracy,Pr.BalAcc,Pr.AUC, ...
                'VariableNames',{'ModelIndex','Permutation','Accuracy','BalAcc','AUC'});

            if mod(pi,20)==0 || pi==cfg.nPerm
                PartialPerm = vertcat(permRows{1:pi});
                writetable(PartialPerm, fullfile(cfg.outDir,sprintf('STEP106_model%d_permutation_partial.csv',mi)));
                fprintf('Model %d perm %d/%d | Acc=%.3f | BalAcc=%.3f | AUC=%.3f\n', ...
                    mi,pi,cfg.nPerm,Pr.Accuracy,Pr.BalAcc,Pr.AUC);
            end
        end

        Perm = vertcat(permRows{:});
        allPerm{end+1,1} = Perm; %#ok<AGROW>

        pAcc = (1 + sum(Perm.Accuracy >= ObsR.Accuracy)) / (cfg.nPerm + 1);
        pBal = (1 + sum(Perm.BalAcc >= ObsR.BalAcc)) / (cfg.nPerm + 1);

        if isfinite(ObsR.AUC)
            pAUC = (1 + sum(Perm.AUC >= ObsR.AUC)) / (cfg.nPerm + 1);
        else
            pAUC = NaN;
        end

        Stat = table(mi,string(model.UnitLevel),string(model.TaskType),string(model.Contrast),model.Run,string(model.Phase),string(model.Classifier),model.K,cfg.nPerm, ...
            ObsR.Accuracy,ObsR.BalAcc,ObsR.AUC, ...
            mean(Perm.Accuracy,'omitnan'),std(Perm.Accuracy,'omitnan'),prctile(Perm.Accuracy,95),pAcc, ...
            mean(Perm.BalAcc,'omitnan'),std(Perm.BalAcc,'omitnan'),prctile(Perm.BalAcc,95),pBal, ...
            mean(Perm.AUC,'omitnan'),std(Perm.AUC,'omitnan'),prctile(Perm.AUC,95),pAUC, ...
            'VariableNames',{'ModelIndex','UnitLevel','TaskType','Contrast','Run','Phase','Classifier','K','Nperm', ...
            'Obs_Accuracy','Obs_BalAcc','Obs_AUC', ...
            'NullMean_Accuracy','NullSD_Accuracy','Null95_Accuracy','p_Accuracy', ...
            'NullMean_BalAcc','NullSD_BalAcc','Null95_BalAcc','p_BalAcc', ...
            'NullMean_AUC','NullSD_AUC','Null95_AUC','p_AUC'});

        allStats{end+1,1} = Stat; %#ok<AGROW>

        writetable(Perm, fullfile(cfg.outDir,sprintf('STEP106_model%d_permutation_null.csv',mi)));
        writetable(Stat, fullfile(cfg.outDir,sprintf('STEP106_model%d_permutation_stats.csv',mi)));

        allStatus{end+1,1} = make_status(model,"ok"); %#ok<AGROW>
        ...
        allStatus{end+1,1} = make_status(model,"failed_" + string(ME.message)); %#ok<AGROW>
    end
end

%% ===================== SAVE ALL =====================

if isempty(allObserved), ObservedAll=table(); else, ObservedAll=vertcat(allObserved{:}); end
if isempty(allPerm), PermAll=table(); else, PermAll=vertcat(allPerm{:}); end
if isempty(allStats), StatsAll=table(); else, StatsAll=vertcat(allStats{:}); end
Status=vertcat(allStatus{:});

writetable(ObservedAll, fullfile(cfg.outDir,'STEP106_observed_all_models.csv'));
writetable(PermAll, fullfile(cfg.outDir,'STEP106_permutation_null_all_models.csv'));
writetable(StatsAll, fullfile(cfg.outDir,'STEP106_permutation_stats_all_models.csv'));
writetable(Status, fullfile(cfg.outDir,'STEP106_status.csv'));

save(fullfile(cfg.outDir,'STEP106_permutation_results.mat'), ...
    'ObservedAll','PermAll','StatsAll','Status','Models','cfg','-v7.3');
...
function S = make_status(model,status)
S = table(string(model.UnitLevel),string(model.TaskType),string(model.Contrast),model.Run,string(model.Phase),string(model.Classifier),model.K,string(status), ...
    'VariableNames',{'UnitLevel','TaskType','Contrast','Run','Phase','Classifier','K','Status'});
end
...
else, error('No DS or T table found.'); end
...
subjects=unique(cleanstr(subjAll));
subjects=subjects(~ismissing(subjects));
nSubj=numel(subjects);

rows=cell(nSubj*nSubj,1);
rr=0;
...
for i=1:nSubj
    ...
    for j=1:nSubj
        ...
        rows{rr}=table(i,sid,role,testSubject, ...
            'VariableNames',{'Fold','SubjectID','Role','LeftOutSubject'});
    end
end

FoldTable=vertcat(rows{:});
foldIDs=unique(FoldTable.Fold);
end
...
s=string(fname);
tf=contains(s,"rie")||contains(s,"pli")||contains(s,"plv")|| ...
...
if isempty(tok)
    tok=regexp(fname,'(?:^|[_-])c0?(\d{1,2})(?:[_-]|$)','tokens');
end
if isempty(tok)
    tok=regexp(fname,'(?:ch|c)0?(\d{1,2})','tokens');
end
...
elseif any(sf==["rms","peak_abs","peak_to_peak","mean","std","variance","tkeo","auc_abs","line_length"])
...
if ismember(ch,1:10)
    roi="frontal";
elseif ismember(ch,11:20)
    roi="frontocentral";
elseif ismember(ch,21:34)
    roi="central";
elseif ismember(ch,35:48)
    roi="centroparietal";
elseif ismember(ch,49:58)
    roi="parietooccipital";
elseif ismember(ch,59:64)
    roi="occipital";
else
    roi="unknown";
end
...
if strcmpi(cfg.taskType,'threeclass')
    ...
    switch string(cfg.contrastName)
        ...
        otherwise
            error('Unsupported binary contrast: %s',cfg.contrastName);
    end
...
for i=1:numel(globalFoldIDs)
    ...
    te=subjTask==testSubj;
    tr=subjTask~=testSubj;
    ...
    k=k+1;
    Folds(k).subject=testSubj;
    Folds(k).foldID=globalFoldIDs(i);
    Folds(k).train=tr;
    Folds(k).test=te;
end
end
...
for s=1:numel(subs)
    ...
    if strcmpi(taskType,'binary')
        if numel(ix)==2 && rand < 0.5
            yPerm(ix)=flipud(yPerm(ix));
        else
...
function [R,P,S]=run_model_once(X,y,subj,Folds,Units,model,cfg)
yt=categorical();
yp=categorical();
scoreAll=[];
...
for fi=1:numel(Folds)
    ...
    if cfg.trainStandardize
        [Xtr,Xte]=train_z(Xtr,Xte);
    end

    [pred,score]=classify_predict(Xtr,ytrain,Xte,string(model.Classifier),cfg);
    ...
    pRows{end+1,1}=table( ...
        repmat(string(model.UnitLevel),sum(te),1), ...
        repmat(string(model.TaskType),sum(te),1), ...
        repmat(string(model.Contrast),sum(te),1), ...
        repmat(model.Run,sum(te),1), ...
        repmat(string(model.Phase),sum(te),1), ...
        repmat(string(model.Classifier),sum(te),1), ...
        repmat(model.K,sum(te),1), ...
        repmat(fi,sum(te),1), ...
        repmat(string(Folds(fi).subject),sum(te),1), ...
        string(ytest),string(pred),double(score(:)), ...
        'VariableNames',{'UnitLevel','TaskType','Contrast','Run','Phase','Classifier','K','Fold','LeftOutSubject','YTrue','YPred','Score'}); %#ok<AGROW>

    Sel=harmonize_selected(Sel,Units);
    Sel.UnitLevel=repmat(string(model.UnitLevel),height(Sel),1);
    Sel.TaskType=repmat(string(model.TaskType),height(Sel),1);
    Sel.Contrast=repmat(string(model.Contrast),height(Sel),1);
    Sel.Run=repmat(model.Run,height(Sel),1);
    Sel.Phase=repmat(string(model.Phase),height(Sel),1);
    Sel.Classifier=repmat(string(model.Classifier),height(Sel),1);
    Sel.K=repmat(model.K,height(Sel),1);
    Sel.Fold=repmat(fi,height(Sel),1);
    Sel.LeftOutSubject=repmat(string(Folds(fi).subject),height(Sel),1);
    Sel=movevars(Sel,{'UnitLevel','TaskType','Contrast','Run','Phase','Classifier','K','Fold','LeftOutSubject'},'Before',1);
...
isThree=string(model.TaskType)=="threeclass";
M=compute_metrics(yt,yp,scoreAll,isThree);

R=table(string(model.UnitLevel),string(model.TaskType),string(model.Contrast),model.Run,string(model.Phase),string(model.Classifier),model.K, ...
    numel(Folds),numel(yt),M.Accuracy,M.BalAcc,M.AUC, ...
    'VariableNames',{'UnitLevel','TaskType','Contrast','Run','Phase','Classifier','K','N_folds','N_predictions','Accuracy','BalAcc','AUC'});

P=vertcat(pRows{:});
S=vertcat(sRows{:});
end

function T=harmonize_selected(T,Units)
vars=T.Properties.VariableNames;

if ~any(strcmp(vars,'Channel'))
    T.Channel=NaN(height(T),1);
else
    T.Channel=double(T.Channel);
end

if ~any(strcmp(vars,'ROI'))
    T.ROI=repmat("NA",height(T),1);
else
    T.ROI=string(T.ROI);
end

if ~any(strcmp(vars,'SpecificFeature'))
    T.SpecificFeature=repmat("NA",height(T),1);
else
    T.SpecificFeature=string(T.SpecificFeature);
end

if ~any(strcmp(vars,'FeatureFamily'))
    T.FeatureFamily=repmat("NA",height(T),1);
else
    T.FeatureFamily=string(T.FeatureFamily);
end

T.UnitID=string(T.UnitID);
end
...
for u=1:nU
    ...
    if numel(x)<3 || numel(categories(removecats(yy)))<2
        continue;
    end

    muAll=mean(x,'omitnan');
    ...
    for c=1:numel(classes)
        ...
        between=between + nc*(muc-muAll)^2;
        within=within + nc*vc;
    end

    if within>eps && isfinite(within)
        FS(u)=between/within;
    end
end
...
B=table();
B.UnitID=string(Units.UnitID);

vars=Units.Properties.VariableNames;

if any(strcmp(vars,'Channel'))
    B.Channel=double(Units.Channel);
else
    B.Channel=NaN(height(Units),1);
end

if any(strcmp(vars,'ROI'))
    B.ROI=string(Units.ROI);
else
    B.ROI=repmat("NA",height(Units),1);
end

if any(strcmp(vars,'SpecificFeature'))
    B.SpecificFeature=string(Units.SpecificFeature);
else
    B.SpecificFeature=repmat("NA",height(Units),1);
end

if any(strcmp(vars,'FeatureFamily'))
    B.FeatureFamily=string(Units.FeatureFamily);
else
    B.FeatureFamily=repmat("NA",height(Units),1);
end
end
...
Xtr(~isfinite(Xtr))=NaN;
Xte(~isfinite(Xte))=NaN;
...
ytr=removecats(categorical(ytr));
cls=categories(ytr);
isMulti=numel(cls)>2;

switch lower(string(clf))
    case "lda"
        M=fitcdiscr(Xtr,ytr,'DiscrimType','linear','ClassNames',cls);
        [pred,score]=predict(M,Xte);

    case "svm"
        if isMulti
            t=templateSVM('KernelFunction','linear','Standardize',false);
            M=fitcecoc(Xtr,ytr,'Learners',t,'ClassNames',cls);
        else
            M=fitcsvm(Xtr,ytr,'KernelFunction','linear','Standardize',false,'ClassNames',cls);
        end
        [pred,score]=predict(M,Xte);

    case "randomforest"
        M=TreeBagger(cfg.nTrees,Xtr,ytr,'Method','classification');
        [pc,score]=predict(M,Xte);
        pred=categorical(string(pc),cls);

    case "decisiontree"
        M=fitctree(Xtr,ytr,'ClassNames',cls);
        [pred,score]=predict(M,Xte);
...
ypred=categorical(ypred,categories(ytrue));

M=struct();
M.Accuracy=mean(ytrue==ypred);

cats=categories(ytrue);
recalls=NaN(numel(cats),1);

for c=1:numel(cats)
    cls=cats{c};
    recalls(c)=sum(ytrue==cls & ypred==cls)/max(sum(ytrue==cls),1);
end
...
if isThree
    ...
    try
        positive=cats{2};
        [~,~,~,M.AUC]=perfcurve(ytrue==positive,score,true);
    catch
        M.AUC=NaN;
    end
end
